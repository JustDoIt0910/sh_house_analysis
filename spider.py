...
class Spider:
    ...
    def get(self, city, region, typ, pages, filename):
        ...
        if typ == Cities.on_sail:
            self.get_on_sail(self.mysql_conn, city, region, pages, filename)
        else:
            self.get_deal(self.mysql_conn, city, region, pages, filename)
        self.mysql_conn.close()

    def get_deal(self, conn, city, region, pages, filename):
                ...
                for deal in info_list:
                    try:
                        ...
                        Utils.add_info(conn, city_id, region_id, 1, infos[0],
                                       infos[1], infos[2][:-2], infos[3], infos[4], infos[5].replace(".", ""))

                    except:
                        continue
                time.sleep(random.randint(2, 5))

    def get_on_sail(self, conn, city, region, pages, filename):
                ...
                for house in info_list:
                    try:
                        ...
                        unitPrice = house.xpath(
                            ".//div[@class='info clear']/div[@class='priceInfo']/div[@class='unitPrice']/span/text()")[0][:-3]
                        unitPrice = unitPrice.replace(",", "")
                        # writer.writerow([region, name, house_type, area, price, unitPrice, None])
                        print(region, name, house_type, area, price, unitPrice)
                        Utils.add_info(conn, city_id, region_id, 0, name, house_type, area, price, unitPrice, "00000000")
...
if __name__ == '__main__':
    ...
    #     writer.writerow(csv_header)
    # # print("-----------获取杭州在售二手房-------------")
    # s.get("杭州", "临平区", Cities.on_sail, 1, filename)
    ...
    #     writer.writerow(csv_header)
    # print("-----------获取济南成交二手房-------------")
    # s.get("济南", "市中", Cities.done, 1, filename)
    # print("-----------获取济南在售二手房-------------")
    # s.get("济南", "槐荫", Cities.on_sail, 2, filename)
    ...
    with open(filename, "w") as f:
        ...
        writer.writerow(csv_header)
    print("-----------获取保定成交二手房-------------")
    s.get("保定", "", Cities.done, 1, filename)
    s.get("保定", "涿州", Cities.done, 1, filename)
    print("-----------获取保定在售二手房-------------")
    s.get("保定", "", Cities.on_sail, 1, filename)
    s.get("保定", "涿州", Cities.on_sail, 1, filename)
    #
    # # 爬开封
    ...
    # 爬杭州
    # filename = "hangzhou_regions.csv"
    # with open(filename, "w") as f:
    #     writer = csv.writer(f)
    #     writer.writerow(csv_header)
    # print("-----------获取西湖在售二手房-------------")
    # s.get("杭州", "西湖", Cities.on_sail, 50, filename)
    # print("-----------获取余杭在售二手房-------------")
    # s.get("杭州", "余杭", Cities.on_sail, 50, filename)
    # print("-----------获取钱塘区在售二手房-------------")
    # s.get("杭州", "钱塘区", Cities.on_sail, 50, filename)
    # print("-----------获取临平区在售二手房-------------")
    # s.get("杭州", "临平区", Cities.on_sail, 50, filename)
    # print("-----------获取拱墅在售二手房-------------")
    # s.get("杭州", "拱墅", Cities.on_sail, 50, filename)
    # print("-----------获取上城在售二手房-------------")
    # s.get("杭州", "上城", Cities.on_sail, 50, filename)
    # print("-----------获取萧山在售二手房-------------")
    # s.get("杭州", "萧山", Cities.on_sail, 50, filename)
